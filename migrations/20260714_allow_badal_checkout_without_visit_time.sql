-- Migration: Allow Omra Badal Stripe checkout without an hourly slot
-- Date: 2026-07-14
...
DECLARE
  ...
  v_is_badal BOOLEAN := LOWER(COALESCE(p_service_name, '')) LIKE '%badal%';
BEGIN
  ...
  IF p_start_date IS NULL OR p_end_date IS NULL THEN
    RAISE EXCEPTION 'Dates de reservation invalides.';
  END IF;

  IF p_end_date < p_start_date THEN
    RAISE EXCEPTION 'La date de fin ne peut pas etre avant la date de debut.';
  END IF;

  IF v_is_badal AND p_end_date <> p_start_date + 6 THEN
    RAISE EXCEPTION 'Pour une Omra Badal, choisissez une fourchette d''une semaine.';
  END IF;

  IF COALESCE(p_pilgrim_charter_accepted, false) = false THEN
    RAISE EXCEPTION 'Vous devez accepter la Charte du Pelerin avant de reserver.';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Reservation impossible: un blocage existe entre vous et ce guide.';
  END IF;
  ...
  ELSE
    IF v_hotel_address IS NULL THEN
      RAISE EXCEPTION 'Adresse de l''hotel requise.';
    END IF;

    IF p_hotel_over_2km_by_car IS NULL THEN
      RAISE EXCEPTION 'Veuillez preciser si l''hotel est a plus de 2 km en voiture.';
    END IF;

    IF p_hotel_over_2km_by_car THEN
      IF p_hotel_distance_km IS NULL OR p_hotel_distance_km <= 2 THEN
        RAISE EXCEPTION 'La distance de l''hotel doit etre strictement superieure a 2 km.';
      END IF;
      v_expected_transport_fee := 10;
    ELSE
      IF p_hotel_distance_km IS NOT NULL THEN
        RAISE EXCEPTION 'Ne renseignez pas de distance si l''hotel est a 2 km ou moins.';
      END IF;
  ...
  IF COALESCE(p_transport_extra_fee_amount, 0) <> v_expected_transport_fee THEN
    RAISE EXCEPTION 'Supplement transport incoherent. Attendu: %.', v_expected_transport_fee;
  END IF;
  ...
  IF ABS(v_total_price - v_expected_total) > 0.01 THEN
    RAISE EXCEPTION 'Montant total incoherent. Attendu: %.', v_expected_total;
  END IF;
  ...
  IF NOT v_is_badal
    ...
    RAISE EXCEPTION 'Une femme ne peut pas reserver seule avec un guide homme. Ajoutez un pelerin homme ou femme pour etre au minimum deux.';
  END IF;
  ...
  VALUES (
    ...
    v_total_price,
    p_location,
    CASE WHEN v_is_badal THEN NULL ELSE p_visit_time END,
    p_pilgrims_names,
    'pending',
    'not_due',
    v_wallet_used,
  ...
  RETURN jsonb_build_object(
    'reservationId', v_reservation.id,
    'walletAmountUsed', v_wallet_used,
    'cardAmountPaid', v_card_paid,
...
BEGIN
  ...
  IF v_card_amount <= 0 THEN
    RAISE EXCEPTION 'Aucun montant carte a payer. Utilisez la cagnotte.';
  END IF;
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'location', p_location,
      'visitTime', CASE WHEN v_is_badal THEN NULL ELSE p_visit_time END,
