-- Migration: Enforce pilgrim charter + block guard in reservation and messaging RPC
-- Date: 2026-03-13
...
DROP FUNCTION IF EXISTS public.create_reservation_with_wallet(UUID, TEXT, DATE, DATE, NUMERIC, TEXT, TEXT, TEXT[], BOOLEAN, TEXT, TEXT, BOOLEAN, NUMERIC, NUMERIC, BOOLEAN, NUMERIC, BOOLEAN, TEXT);
...
DROP FUNCTION IF EXISTS public.send_message_with_guard(UUID, TEXT);
CREATE OR REPLACE FUNCTION public.send_message_with_guard(
  p_receiver_id UUID,
  p_content TEXT
)
RETURNS JSONB
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM public.user_blocks ub
    WHERE (ub.blocker_id = v_user_id AND ub.blocked_id = p_receiver_id)
      OR (ub.blocker_id = p_receiver_id AND ub.blocked_id = v_user_id)
  ) THEN
    RAISE EXCEPTION 'Conversation bloquée entre ces utilisateurs.';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'is_read', COALESCE(v_message.is_read, false),
    'created_at', v_message.created_at
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.send_message_with_guard(UUID, TEXT) TO authenticated;
