-- Migration: Fixed transport pickup logic (Haram/Hotel + 2km rule)
-- Date: 2026-03-01

ALTER TABLE public.reservations
  ADD COLUMN IF NOT EXISTS transport_pickup_type TEXT,
  ADD COLUMN IF NOT EXISTS hotel_address TEXT,
  ADD COLUMN IF NOT EXISTS hotel_over_2km_by_car BOOLEAN,
  ADD COLUMN IF NOT EXISTS hotel_distance_km NUMERIC,
  ADD COLUMN IF NOT EXISTS transport_extra_fee_amount NUMERIC NOT NULL DEFAULT 0;
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'reservations_transport_pickup_type_check'
      AND conrelid = 'public.reservations'::regclass
  ) THEN
    ALTER TABLE public.reservations DROP CONSTRAINT reservations_transport_pickup_type_check;
  END IF;

  ALTER TABLE public.reservations
    ADD CONSTRAINT reservations_transport_pickup_type_check
      CHECK (
        transport_pickup_type IS NULL
        OR transport_pickup_type IN ('haram', 'hotel')
      );
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'reservations_transport_extra_fee_amount_check'
      AND conrelid = 'public.reservations'::regclass
  ) THEN
    ALTER TABLE public.reservations DROP CONSTRAINT reservations_transport_extra_fee_amount_check;
  END IF;

  ALTER TABLE public.reservations
    ADD CONSTRAINT reservations_transport_extra_fee_amount_check
      CHECK (COALESCE(transport_extra_fee_amount, 0) IN (0, 10));
EXCEPTION WHEN duplicate_object THEN
  NULL;
END $$;
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'reservations_transport_hotel_coherence_check'
      AND conrelid = 'public.reservations'::regclass
  ) THEN
    ALTER TABLE public.reservations DROP CONSTRAINT reservations_transport_hotel_coherence_check;
  END IF;

  ALTER TABLE public.reservations
    ADD CONSTRAINT reservations_transport_hotel_coherence_check
      CHECK (
        transport_pickup_type IS NULL
        OR (
          transport_pickup_type = 'haram'
          AND hotel_address IS NULL
          AND hotel_over_2km_by_car IS NULL
          AND hotel_distance_km IS NULL
          AND COALESCE(transport_extra_fee_amount, 0) = 0
        )
        OR (
          transport_pickup_type = 'hotel'
          AND NULLIF(BTRIM(COALESCE(hotel_address, '')), '') IS NOT NULL
          AND hotel_over_2km_by_car IS NOT NULL
          AND (
            (
              hotel_over_2km_by_car = true
              AND hotel_distance_km IS NOT NULL
              AND hotel_distance_km > 2
              AND COALESCE(transport_extra_fee_amount, 0) = 10
            )
            OR (
              hotel_over_2km_by_car = false
              AND hotel_distance_km IS NULL
              AND COALESCE(transport_extra_fee_amount, 0) = 0
            )
          )
...
DROP FUNCTION IF EXISTS public.create_reservation_with_wallet(UUID, TEXT, DATE, DATE, NUMERIC, TEXT, TEXT, TEXT[], BOOLEAN);
