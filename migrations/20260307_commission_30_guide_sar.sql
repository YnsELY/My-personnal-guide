-- Migration: Commission 30% + guide payout basis + commissionable net amount
-- Date: 2026-03-07

ALTER TABLE public.reservations
  ADD COLUMN IF NOT EXISTS commissionable_net_amount NUMERIC;
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'reservations_commissionable_net_amount_non_negative_check'
      AND conrelid = 'public.reservations'::regclass
  ) THEN
    ALTER TABLE public.reservations DROP CONSTRAINT reservations_commissionable_net_amount_non_negative_check;
  END IF;

  ALTER TABLE public.reservations
    ADD CONSTRAINT reservations_commissionable_net_amount_non_negative_check
      CHECK (commissionable_net_amount IS NULL OR commissionable_net_amount >= 0);
...
WITH normalized AS (
  SELECT
    ...
    ROUND(
      GREATEST(
        COALESCE(commissionable_net_amount, GREATEST(COALESCE(total_price, 0) - COALESCE(transport_extra_fee_amount, 0), 0)),
        0
      )::numeric,
      2
    ) AS commissionable_net
...
SET
  ...
  commissionable_net_amount = n.commissionable_net,
  platform_fee_amount = ROUND((n.commissionable_net * 0.30)::numeric, 2),
  guide_net_amount = ROUND((COALESCE(r.total_price, 0) - ROUND((n.commissionable_net * 0.30)::numeric, 2))::numeric, 2),
  updated_at = NOW()
FROM normalized n
WHERE r.id = n.id;
