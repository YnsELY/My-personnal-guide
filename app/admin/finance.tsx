        ...
        <AdminGuard>
            <View className="flex-1 bg-gray-50 dark:bg-zinc-900">
                        ...
                        <FlatList
                            ...
                            ListHeaderComponent={() => (
                                <View className="mb-4">
                                    <View className="bg-white dark:bg-zinc-800 border border-gray-100 dark:border-white/10 rounded-2xl p-4 mb-3">
                                        ...
                                        <Text className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{formatCurrency(finance?.gmv || 0)}</Text>
                                        <View className="flex-row justify-between mt-3">
                                            <View>
                                                <Text className="text-gray-400 text-xs">Revenu plateforme</Text>
                                                <Text className="text-green-500 font-semibold">{formatCurrency(finance?.platformRevenue || 0)}</Text>
                                            </View>
                                            <View>
                                                <Text className="text-gray-400 text-xs text-right">Net guides</Text>
                                                <Text className="text-[#b39164] font-semibold text-right">{formatCurrency(finance?.netToGuides || 0)}</Text>
                                            </View>
                                        </View>
                                    </View>
                                    ...
                                    <View className="flex-row gap-3">
                                        <SummaryCard
                                            label="Déjà payé"
                                            value={formatCurrency(finance?.paidToGuides || 0)}
                                            icon={<CreditCard size={16} color="#22c55e" />}
                                        />
                                        <SummaryCard
                                            label="Reste à payer"
                                            value={formatCurrency(finance?.dueToGuides || 0)}
                            ...
                            renderItem={({ item }) => (
                                <View className="bg-white dark:bg-zinc-800 border border-gray-100 dark:border-white/10 rounded-2xl p-4 mb-3">
                                    ...
                                    <View className="mt-3">
                                        <FinanceRow label="CA guide" value={formatCurrency(item.grossAmount)} />
                                        <FinanceRow label="Commission plateforme" value={formatCurrency(item.platformFeeAmount)} />
                                        <FinanceRow label="Net guide" value={formatCurrency(item.netAmount)} />
                                        <FinanceRow label="Déjà payé" value={formatCurrency(item.paidAmount)} />
                                        <FinanceRow label="Reste à payer" value={formatCurrency(item.dueAmount)} highlight />
                                    </View>
                ...
                <SlideToConfirmModal
                    ...
                    message={`Marquer ${pendingPayoutGuide?.guideName || 'ce guide'} comme payé (${formatCurrency(pendingPayoutGuide?.dueAmount || 0)}) ?`}
